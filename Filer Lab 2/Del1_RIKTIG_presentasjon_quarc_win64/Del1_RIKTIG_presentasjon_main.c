...
typedef RT_MODEL_Del1_RIKTIG_presenta_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Del1_RIKTIG_presentasjon(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Del1_RIKTIG_presentasjon_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Del1_RIKTIG_presentasjon_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://Del1_RIKTIG_presentasjon:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Del1_RIKTIG_presentasjon -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        result = hil_task_start
          (Del1_RIKTIG_presentasjon_DW.HILReadEncoderTimebase_Task, (t_clock)
           Del1_RIKTIG_presentasjon_P.HILReadEncoderTimebase_clock, 500.0, -1)
          ;
        if (result == 0) {
